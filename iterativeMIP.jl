...
function best_n_reward_MIP(rutas, recompensas, n_vehicles)
    # Crear modelo con el solver GLPK
    modelo = Model(GLPK.Optimizer)
    ...
    best_n_reward = sort([recompensas[i] for i in 1:length(rutas) if value(x[i]) > 0.5])

    return best_n_reward[n_vehicles]
end
