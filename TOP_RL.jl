...
function simulation_rl(edges::Dict{Int8, Dict{Int8, Float64}}, num_simulations::Int, capacity::Float64, routes::Array{Route,1})
    avg_reward = []
    for _ in 1:num_simulations
        reward_route = []
        for route in routes
...
function merge_routes_rl(Node1::Node, Node2::Node, edges::Dict{Int8, Dict{Int8, Float64}}, routes::Dict{Int, Route}, max_distance::Float64, rl)
    route1 = routes[Node1.route_id]
    route2 = routes[Node2.route_id]
...
function heuristic_with_BR_RL(n_vehicles, nodes, edges::Dict{Int8, Dict{Int8, Float64}}, capacity, alpha, beta, savings, rl)
    routes = dummy_solution(nodes, edges, capacity)
    savings = reorder_saving_list(savings, beta)
    ...
    for key in keys(savings)
        NodeX = nodes[key[1]]
        NodeY = nodes[key[2]]
        merge_routes_rl(NodeX, NodeY, edges, routes, capacity, rl)
    end
    sorted_routes = sort(collect(routes), by = x -> x[2].reward, rev = true)
    sorted_routes = Dict(kv[1] => kv[2] for kv in sorted_routes)
