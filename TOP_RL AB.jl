...
function main()
    ...
    n_vehicles, capacity, nodes = parse_txt("Instances/Set_102_234/p7.2.l.txt")
    start_node = 1
    ...
    list_savings_dict_alpha = Dict{Float64,OrderedDict{Tuple{Int, Int}, Float64}}()
    for i=1:9
        list_savings_dict_alpha[i/10] = calculate_savings_dict(nodes, edges, Float16(i/10))
    end
    ...
    @time begin
        for iter in 1:5000
            ...
            if reward > Param_dict[(alpha,beta)][2]
